...
export default function HomeProfessor({ usuario, onLogout }) {
  ...
  // Card 2 (AGORA USA MATRÍCULA)
  const [vincularDiscId, setVincularDiscId] = useState('');
  const [vincularMatricula, setVincularMatricula] = useState(''); // <--- MUDOU AQUI
  ...
  const buscarDisciplinas = async () => {
    try {
      const res = await fetch(`${ASSIGN_API}/api/subjects/professor/${usuario.id}`);
      if (res.ok) {
        const data = await res.json();
        setMinhasDisciplinas(data);
        if (data.length > 0 && !vincularDiscId) {
          setVincularDiscId(data[0].id.toString());
          setTarefaDiscId(data[0].id.toString());
        }
      }
  ...
  const limparAvisos = () => { setErroGeral(''); setMsgDiscSucesso(''); setMsgVincSucesso(''); setMsgTarefaSucesso(''); };

  // 1. CRIAR MATÉRIA
  const handleCriarDisciplina = async (e) => {
    e.preventDefault(); limparAvisos(); setCarregando(true);
    try {
      const res = await fetch(`${ASSIGN_API}/api/subjects`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nome: nomeNovaDisciplina, professor_id: usuario.id })
      });
      const data = await res.json();
      if (res.status === 201) { setMsgDiscSucesso(data.mensagem); setNomeNovaDisciplina(''); buscarDisciplinas(); } 
      else setErroGeral(data.erro);
    } catch (err) { setErroGeral('Erro de rede.'); } finally { setCarregando(false); }
  };

  // 2. MATRICULAR ALUNO PELA MATRÍCULA
  const handleMatricularAluno = async (e) => {
    e.preventDefault(); limparAvisos(); setCarregando(true);
    try {
      const res = await fetch(`${ASSIGN_API}/api/subjects/enroll`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          disciplina_id: vincularDiscId, 
          matricula: vincularMatricula // <--- DESPACHANDO A MATRÍCULA
        })
      });
      const data = await res.json();
  ...
  const handleDispararAtividade = async (e) => {
    ...
    try {
      const res = await fetch(`${ASSIGN_API}/api/assignments/broadcast`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ disciplina_id: tarefaDiscId, titulo: tarefaTitulo })
      });
      const data = await res.json();
  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        <div><h2 style={{ margin: 0, color: '#ffb703' }}>Painel Docente</h2><span style={{ fontSize: '12px', color: '#bbb' }}>Prof. {usuario.nome}</span></div>
      ...
      <div style={styles.grid}>
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleCriarDisciplina} style={styles.form}>
            <div><label style={styles.label}>Nome da matéria</label><input type="text" required placeholder="Ex: Algoritmos" value={nomeNovaDisciplina} onChange={e => setNomeNovaDisciplina(e.target.value)} style={styles.input} /></div>
          ...
          <form onSubmit={handleMatricularAluno} style={styles.form}>
            ...
            <div>
              ...
              <input 
                type="text" 
                required 
                placeholder="Ex: 20261045050012" 
                value={vincularMatricula} 
                onChange={e => setVincularMatricula(e.target.value)} 
                style={styles.input} 
              />
            </div>
      ...
      <div style={{ ...styles.card, marginTop: '20px' }}>
        ...
        <form onSubmit={handleDispararAtividade} style={{ display: 'flex', gap: '15px', alignItems: 'flex-end' }}>
          <div style={{ flex: 1 }}><label style={styles.label}>Turma Destino</label><select value={tarefaDiscId} onChange={e => setTarefaDiscId(e.target.value)} style={styles.input} disabled={minhasDisciplinas.length === 0}>{minhasDisciplinas.map(d => <option key={d.id} value={d.id}>{d.nome}</option>)}</select></div>
          <div style={{ flex: 2 }}><label style={styles.label}>Tarefa</label><input type="text" required placeholder="Ex: Ler o capítulo 3" value={tarefaTitulo} onChange={e => setTarefaTitulo(e.target.value)} style={styles.input} /></div>
